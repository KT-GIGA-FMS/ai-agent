...
def check_availability(from_time: str, to_time: str, fuel_type: str|None=None, car_type: str|None=None) -> list:
    ...
    try:
        ...
        if fuel_type: 
            cand = [c for c in cand if (c.get("fuel_type") or "").lower()==fuel_type.lower()]
        if car_type: 
            cand = [c for c in cand if (c.get("car_type") or "").lower()==car_type.lower()]
...
def create_reservation(user_id: str, vehicle_id: str, from_time: str, to_time: str, idem_key: str|None=None) -> dict:
    ...
    try:
        ...
        if not car:
            return {
                "error": "차량을 찾을 수 없습니다",
                "available_vehicles": [{"id": c["id"], "name": MODELS.get(c["car_model_id"], {}).get("name", "Unknown")} for c in CARS[:5]]
            }
        ...
        # 시간 충돌 체크
        for r in RESV:
            if (r["vehicle_id"] == actual_vehicle_id and 
                r["status"] in ("confirmed","in_progress") and 
                _overlap(s,e,r["start_at"],r["end_at"])):
